...
useEffect(() => {
  ...
  const player = ecsData.players.find((item) => item.id === selectedPlayerId) || ecsData.players[0];
  ...
  const filteredPlayers = useMemo(() => {
    ...
    if (!normalized) return ecsData.players;

    return ecsData.players.filter((item) =>
      [item.nick, item.team, item.telegram, item.epic, item.steam].join(" ").toLowerCase().includes(normalized),
    );
  }, [query]);
    ...
    <div className="app-shell" data-game={game}>
      <aside className="sidebar">
        ...
        <section className="mini-profile">
          ...
          <div>
            <strong>{ecsData.user.name}</strong>
            <span>Уровень 24</span>
          </div>
      ...
      <main className="main">
        ...
        {view === "dashboard" && (
          <section className="view active">
            ...
            <div className="dashboard-grid">
              ...
              <LeaderboardPreview game={game} leaderboard={leaderboard} onSelect={setSelectedPlayerId} openPlayers={() => setView("players")} />
        ...
        {view === "players" && (
          <Page title="Игроки и статистика" eyebrow="Player database">
            <input className="search-input" type="search" placeholder="Ник, Telegram, Epic или Steam" value={query} onChange={(event) => setQuery(event.target.value)} />
            <div className="players-layout">
              <section className="panel">
                <div className="player-results">
                  {filteredPlayers.map((item) => (
                    <button key={item.id} className={`player-row ${item.id === selectedPlayerId ? "active" : ""}`} onClick={() => setSelectedPlayerId(item.id)}>
                      <span>
                        <strong>{item.nick}</strong>
                        <span>{item.telegram}</span>
                      </span>
                      <span className="pill">{game === "cs2" ? item.cs2Rating : item.fortnitePoints}</span>
                    </button>
                  ))}
                </div>
              </section>
          ...
          <Page title="Таблица лидеров" eyebrow="Season ranking">
            <div className="panel leaderboard-panel">
              {leaderboard.map((item, index) => (
                <button key={item.id} className="leader-row" onClick={() => { setSelectedPlayerId(item.id); setView("players"); }}>
                  <span className="leader-rank">#{index + 1}</span>
                  <span className="leader-main">
                    <strong>{item.nick}</strong>
                    <span className="leader-meta">{item.telegram}</span>
                  </span>
...
function TournamentCode({ tournament, registeredTournamentId, register }) {
  ...
  return (
    <>
      <div className="condition-row"><span>Подписка на Telegram</span><strong className="result-win">Проверено</strong></div>
...
function PlayerDetails({ game, player }) {
  return (
    <section className="panel">
      <div className="player-details-head">
        <div>
          ...
          <h2>{player.nick}</h2>
          <p className="muted">{player.team} / уровень {player.level}</p>
        </div>
        <img className="avatar" src="/assets/avatar-akkerman.png" alt={player.nick} />
      </div>
      <div className="identity-grid">
        ...
        <div><span>ECS rating</span><strong>{game === "cs2" ? player.cs2Rating : player.fortnitePoints}</strong></div>
      </div>
      <div className="external-card">
        <p className="eyebrow">{game === "cs2" ? "CS2 stats" : "FortniteTracker ready"}</p>
